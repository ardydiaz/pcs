<?php
    ...
    $facultyCourseOptions = $facultyCourses->map(function ($facultyCourse) {
        ...
        $label = $course ? trim($courseCode . ($courseSubject ? ' - ' . $courseSubject : '')) : 'Course #' . $facultyCourse->id;
        $termParts = array_filter([$formattedAcademicYear, $semesterDisplay], function ($value) {
            return trim($value ?? '') !== '';
        });
        $termLabel = implode(' • ', $termParts);
        $searchTokens = array_filter([$label, $facultyRawName, $academicYear, $formattedAcademicYear, $semesterRaw, $semesterDisplay], function ($value) {
            return trim($value ?? '') !== '';
        });
...
<?php $__env->startSection('content'); ?>
        ...
        <div class="card schedule-card schedule-card--table mb-4 schedule-action-card">
            <div class="card-body d-flex flex-column flex-lg-row align-items-center justify-content-between gap-3">
                ...
                <div class="d-flex align-items-center gap-3 flex-wrap justify-content-center">
                    <?php if($canAdd): ?>
                        <button type="button" class="btn btn-schedule-action" data-bs-toggle="modal" data-bs-target="#scheduleCreateModal">
                ...
                <div class="row g-3 align-items-center">
                    <div class="col-md-6">
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            ...
                            <select id="schedulesRowsPerPage" class="form-select schedule-page-size fw-bold" style="width: auto;" data-table-length>
                    ...
                    <div class="col-md-6">
                        <div class="d-flex justify-content-end align-items-center gap-2 flex-wrap">
                            <div class="dropdown table-filter-dropdown">
                                <button class="filter-toggle" type="button" id="scheduleFilterToggle" data-bs-toggle="dropdown" aria-expanded="false">
                                    <span>Filters</span>
                                ...
                                <div class="dropdown-menu dropdown-menu-end p-3">
                                    ...
                                    <div class="d-flex justify-content-end">
                                        <button type="button" class="btn btn-link p-0 table-filter-reset" id="scheduleFilterReset">Reset Filters</button>
                                    </div>
                                </div>
                            </div>
                            <div class="schedule-search-wrapper">
                                <i class="bx bx-search schedule-search-icon"></i>
                                <input type="text" id="schedulesSearch" class="schedule-search-input" data-table-search placeholder="Search...">
            ...
            <div class="card-body pt-0">
                            ...
                            <tr>
                                <?php if($canDelete || $showDeleteDisabled): ?>
                                    <th class="text-center">
                                        <input type="checkbox" class="form-check-input schedule-checkbox" id="scheduleSelectAll">
                                    </th>
                                <?php endif; ?>
                ...
                <?php if($canDelete || $showDeleteDisabled): ?>
                    <div class="schedule-bulk-bar d-none" id="scheduleBulkBar">
                        ...
                        <?php if($canDelete): ?>
                            <button type="button" class="schedule-bulk-btn schedule-bulk-btn--danger" data-bulk-action="delete">
                                <i class="bx bx-trash"></i> Delete
                            </button>
                        <?php else: ?>
                            <button type="button" class="schedule-bulk-btn schedule-bulk-btn--danger disabled" disabled aria-disabled="true">
                                <i class="bx bx-trash"></i> Delete
                            </button>
                        <?php endif; ?>
            ...
            <div class="modal-content schedule-card">
                <form id="createScheduleForm">
                    ...
                    <div class="modal-header schedule-modal-header">
                        <h5 class="modal-title mb-0" id="scheduleCreateModalLabel">Add Schedule</h5>
                    ...
                    <div class="modal-body schedule-modal-body">
                        <div class="mb-3">
                            <label class="form-label" for="createFacultyCourse">Faculty Course</label>
                            <div class="dropdown w-100 course-dropdown" data-course-dropdown>
                                    ...
                                    <div class="course-dropdown-list" data-course-list>
                                        <?php $__currentLoopData = $facultyCourseOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $option): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <button type="button"
                                                ...
                                                <?php if(!empty($option['faculty']) || !empty($option['academic_year_display']) || !empty($option['semester'])): ?>
                                                    <small class="course-dropdown-info" title="<?php echo e(trim(implode(' | ', array_filter([$option['faculty'], $option['academic_year_display'], $option['semester']])))); ?>">
                                ...
                                <input type="hidden" name="faculty_course_id" id="createFacultyCourse" required>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="createTimeStart">Time</label>
                            <div class="time-range-group">
                                <div class="dropdown time-dropdown" data-time-dropdown>
                                    <button class="time-dropdown-toggle"
                                            type="button"
                                            id="createTimeStartToggle"
                                            data-bs-toggle="dropdown"
                                            data-bs-display="static"
                    ...
                    <div class="modal-footer schedule-modal-footer">
                        ...
                        <button type="submit" class="btn btn-schedule-primary schedule-modal-btn" data-default-text="Save" data-loading-text="Saving...">Save</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal fade" id="scheduleImportModal" tabindex="-1" aria-labelledby="scheduleImportModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered schedule-modal-dialog schedule-modal-dialog--narrow">
            <div class="modal-content schedule-card">
                <form method="POST" action="<?php echo e(route('dm.schedules.import')); ?>" enctype="multipart/form-data" id="scheduleImportForm">
                    <?php echo csrf_field(); ?>
                    <div class="modal-header schedule-modal-header">
...
<?php $__env->startSection('page-script'); ?>
    ...
    <script>
        ...
        class SchedulePage {
            constructor({ schedules, facultyCourses, tableRoot }) {
            ...
            buildRow(schedule) {
                ...
                const actionsCell = (schedulePermissions.canEdit || schedulePermissions.canDelete || schedulePermissions.showDeleteDisabled)
                    ? `
            ...
            async handleUpdateSchedule(form, scheduleId, data) {
                ...
                try {
                    const response = await fetch(`<?php echo e(route('dm.schedules.update', ':id')); ?>`.replace(':id', scheduleId), {
                        method: 'PUT',
                        headers: this.defaultHeaders(),
                        body: JSON.stringify(data),
                    });
            ...
            async executeDeleteSchedule(button, scheduleId) {
                ...
                try {
                    const response = await fetch(`<?php echo e(route('dm.schedules.destroy', ':id')); ?>`.replace(':id', scheduleId), {
                        method: 'DELETE',
...
<?php echo $__env->make('layouts/contentNavbarLayout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/postclasssurvey.mcu.edu.ph/resources/views/content/data-management/dm-schedules.blade.php ENDPATH**/ ?>